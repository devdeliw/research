...
image_path_rectangle = '/Users/devaldeliwala/research/muLab/work/plots&data/red_clump_analysis/rectangle_plots/'

# for the riemann software:
image_path_riemann = '/Users/devaldeliwala/research/muLab/work/plots&data/red_clump_analysis/riemann_plots/'

#-------------------------------------------------------------------------------------------------#

# access script 
script = pd.read_csv("ocf-shortened-parameter-sheet.csv")
script2 = pd.read_csv("slope-data.csv") 
starting_row = 2
starting_row -= 2
...
for i in ['x_range', 'parallel_cutoff1', 'parallel_cutoff2']: 
    unprocessed_script.loc[:, i] = unprocessed_script.loc[:, i].apply(lambda x: literal_eval(x))



# matched star catalog
fits ='/Users/devaldeliwala/research/muLab/work/catalogs/dr2/jwst_init_NRCB.fits'
rc_path = '/Users/devaldeliwala/research/muLab/work/rc-magnitudes.csv'
...
for index, row in unprocessed_script.iterrows():
	...
	if not sub_populations: 
		...
		#if 	(catalog1name, catalog2name) != ('F212N', 'F323N'):

			# if bootstrap set to True, use bootstrapping to calculate 
			# error on the means -- note this takes *much* longer to run
		if xlim == 0: 
			slope, d_slope, slope_fritz, inter = class_.run(ns = ns, bootstrap = False)
		else: 
			slope, d_slope, slope_fritz, inter = class_.run(ns = ns)

		#else: 
		#	slope, d_slope, slope_fritz = class_.run_rc(ns = ns, bootstrap = False)

		unprocessed_script.loc[index, 'slope_2'] = slope 
		unprocessed_script.loc[index, 'd_slope_2'] = d_slope
		unprocessed_script.loc[index, 'slope_inter_2'] = slope_fritz
		unprocessed_script.loc[index, 'slope_fritz_2'] = slope_fritz

		# output script with final calculated slopes and errors
		updated_df = pd.concat([processed_script, unprocessed_script])
		updated_df.to_csv('slope-data_.csv', index=False)
