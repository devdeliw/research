...
class Analysis: 
    ...
    def slope(self, show_plot = True):
        ...
        if show_plot: 
            ...
            if self.catalogyname == self.catalog1name: 
                ...
                mask = np.zeros(len(self.catalog2), dtype=bool)

                mask[(x >= lower_xlim) & (x <= upper_xlim) & 
                     (self.catalog1 <= max(means) + 0.5) & 
                     (self.catalog1 >= min(means) - 0.5)] = True
            ...
            if self.catalogyname == self.catalog2name: 
                ...
                mask[(x >= lower_xlim) & (x <= upper_xlim) & 
                     (self.catalog2 <= max(means) + 0.5) & 
                     (self.catalog2 >= min(means) - 0.5)] = True

                x = x[mask]
                y = self.catalog2[mask]
                """
                k = gaussian_kde([x,y])
                xi, yi = np.mgrid[
                   x.min():x.max():nbins*1j,
            ...
            plt.xlabel(f"{self.catalog1name} - {self.catalog2name}", fontsize = 15)
            plt.plot(midpoints, [inter + i * slope for i in midpoints], '-', label = 'linear fit', color = mcolors.CSS4_COLORS['mediumspringgreen'], linewidth = 2)
            plt.scatter(midpoints, means, color= mcolors.CSS4_COLORS['mediumspringgreen'], s = 20, marker = 's', label = 'means')

            errors = [error * 5 for error in errors]
            plt.errorbar(midpoints, means, yerr=errors, color= mcolors.CSS4_COLORS['mediumspringgreen'], capsize=2, capthick=1, lw = 1, ls = 'none')

            plt.legend()
            plt.gca().invert_yaxis()
            plt.title(f"Calculated Slope: {slope.round(3)} ± {d_slope.round(3)} | Fritz Slope: {slope_fritz.round(3)} | Δ Slope: {abs(slope - slope_fritz).round(3)} | Errorbars scaled 5x. ", fontsize = 15)
    ...
    def run_optimal_bin(self, ns, show_hists = False, perform_plot = True, perform_residuals = False, perform_KS = False):
        ...
        return self.slope_value, self.d_slope_value, self.slope_fritz
...
class Run_Riemann: 
    ...
    def run_rc(self, ns, bootstrap = False, perform_plot = True, perform_residuals = False, perform_KS = False, 
