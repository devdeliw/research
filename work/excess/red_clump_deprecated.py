...
class Red_Clump_Analysis_vRiemann:
    def __init__(self, catalog1, catalog2, 
        ...
        self.larger_x_range = larger_x_range
        self.n = n
        self.image_path = image_path 
    ...
    def display_cutoffs(self, verbose = False): 
        ...
        if self.larger_x_range: 
            plt.axvline(x = self.larger_x_range[0], c = 'r', linestyle = ':', alpha = 0.5, label = 'selected x range')
            plt.axvline(x = self.larger_x_range[1], c = 'r', linestyle = ':', alpha = 0.5)
    ...
    def generate_tile_bins(self, show_plot = True, verbose = False):
        ...
        yi = slope * self.larger_x_range[0] + intercept1
        yf = slope * self.larger_x_range[0] + intercept2

        bins.append([[self.larger_x_range[0], self.x_range[0]], [yi, yf]])
        segments.append(Rectangle((self.larger_x_range[0], yi), self.x_range[0]-self.larger_x_range[0], height, 
                                      facecolor = (0, 0, 0, 0), lw = 2, 
                                      ec = (1, 0, 0, 1)
                        )
        )

        yi = slope * self.x_range[1] + intercept1
        yf = slope * self.x_range[1] + intercept2

        bins.append([[self.x_range[1], self.larger_x_range[1]], [yi, yf]])
        segments.append(Rectangle((self.x_range[1], yi), self.larger_x_range[1]-self.x_range[1], height, 
                                  facecolor = (0, 0, 0, 0), lw = 2, 
                                  ec = (1, 0, 0, 1)
                        )
        )
    ...
    def generate_tile_hists(self, verbose = False, show_hists = False):
        ...
        for i in range(len(starlist)): 
            ...
            if self.catalogyname == self.catalog2name: 
                ...
                # fitting 
                optimized_bin_value, optimized_amplitude, optimized_mean, optimized_error, optimized_std = 0, 0, 0, 0, 0
...
def riemann_result(catalog, catalog1filt, catalog2filt, catalogyfilt, 
                   ...
                   parallel_cutoff1, parallel_cutoff2, 
                   x_range, larger_x_range, n, image_path, hists = False,
    ...
    riemann_class = Red_Clump_Analysis_vRiemann(catalog1, catalog2, 
                                ...
                                x_range = x_range, larger_x_range = larger_x_range,
                                n = n,
