    ...
    def optimize_bin(self): 
        ...
        for i in range(8, 20): 
            ...
            while not amplitude_works: 
                ...
                if fitted_std > 0.1 and fitted_std < maximum_allowed_std and fitted_amplitude < 500 and fitted_amplitude > 0: 
...
class Analysis: 
    ...
    catalog1name, catalog2name, cataloyname: str
    ...
    parallel_cutoff1, parallel_cutoff2: [(float, float), (float, float)]
    ...
    def n_analysis(self, ns): 
        ...
        It fins the optimal number of rough segments such that the error of the slope is
        minimized. 
    ...
    def analysis(self, show_hists=False): 
        ...
        while current_x <= self.x_range[1]: 
            ...
            while not status: 
                ...
                if count > 5: 
                    raise Exception("optimize_bin() never found a suitable fitting. \
                                     Try altering `x_range` or squeezing the `parallel_cutoff`s tighter to RC bar; that usually works."
                          )
...
class Run_Riemann: 
    ...
    def sub_populations(self, n, ns, image_path = False, show_populations = True):
        ...
        if show_populations: 
            ...
            plt.xlabel(f"{self.catalog1name} - {self.catalog2name}")
            plt.ylabel(f"{self.catalogyname}")
