...
from red_clump_analysis import riemann_result
import os
...
x_range_ = { 
	...
	'F115W' : [None, [5.8, 9], [6, 10], [7.5, 11.5]], 
	'F212N' : [None, None, None, [1.3, 2.6]], 
...
x_range_ = pd.DataFrame(x_range_).set_index('x1')

fits ='~/research/work/catalogs/dr2/jwst_init_NRCB.fits'
catalog = Table.read(fits, format='fits')

# Add what you want to run. [[[x1, x2, y]], ...] for an x1-x2 vs y CMD
...
for i in range(len(script)): 
	...
	x_range = x_range_[catalog1filt][catalog2filt]
	catalog1zp = zeropoints[catalog1filt][region1]
	catalog2zp = zeropoints[catalog2filt][region2]

	n = 10
	image_path = f"/Users/devaldeliwala/research/work/plots&data/rc_analysis_plots/{region1}/{catalog1filt}-{catalog2filt}/vs{catalogyfilt}/"
	hists = [True, False]
